      ...
      try {
        ...
        if (categories) {
          ...
          categories.forEach(cat => {
            const categoryProducts = productsData.filter(product => product.category === cat);
            if (categoryProducts.length > 0) {
              groupedProducts[cat] = limit ? categoryProducts.slice(0, limit) : categoryProducts;
            }
          });
        } else if (category) {
          // For homepage with single category
          const categoryProducts = productsData.filter(product => product.category === category);
          groupedProducts[category] = limit ? categoryProducts.slice(0, limit) : categoryProducts;
        } else {
